...
قبل النشر، يجب أن ينجح كل من تدقيق BAI (`bai_audit.gate_pass` مع دليل قابل للتتبع)، وفحص الأمان، وفحص الملاءمة. فشل أي شرط يوجّه الحالة إلى المراجعة البشرية.
...
استورد `workflow.json` من قائمة الاستيراد في n8n وشغّله عبر `Manual Trigger`. البيانات التجريبية مضبوطة على `A = true` و`B = false` و`S = true`، وتحتوي على تدقيق BAI ناجح مع `evidence_refs`، ولذلك تكون النتيجة `AUTO_PUBLISH`.

لتجربة مسار المراجعة، غيّر `safetyPass` إلى `false` أو غيّر `bai_audit.gate_pass` إلى `false` أو احذف `evidence_refs`. حتى إذا جعلت فحصي الملاءمة صحيحين، سيبقى القرار `HUMAN_REVIEW` لأن الأمان شرط إلزامي. ويمكنك أيضًا جعل `brandFit` و`platformFit` كلاهما `false` مع إبقاء `safetyPass` صحيحًا لرؤية فشل الملاءمة.
